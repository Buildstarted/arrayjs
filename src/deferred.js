...
//		},
		...
		//orderByDescending
		orderByDescending: function(predicate, defaultValue) {
			var baseDelegate = this.__iterator__;
			return new ArrayJs.fn.init(function() {
				...
				if (comparer) {
					sortedKeys = values.select(keySelector).distinct().sort(comparer).reverse();
				} else {
					sortedKeys = values.select(keySelector).distinct().sort().reverse();
				}
				
				for(var i in sortedKeys) {
					for(var c in values.where(function(s) { keySelector(s) === sortedKeys[i]; })) {
						yield c;
					}
				}
			});
		...
		//orderBy
		orderBy: function(predicate, defaultValue) {
			var baseDelegate = this.__iterator__;
			return new ArrayJs.fn.init(function() {
				...
				if (comparer) {
					sortedKeys = values.select(keySelector).distinct().sort(comparer);
				} else {
					sortedKeys = values.select(keySelector).distinct().sort();
				}
